...
/*   Updated: 2022/01/13 19:56:07 by rkaufman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_apply_right_aligned(char *prefix, char *s, t_flags *flags)
{
	...
	if (ft_strchr("cspdiuxX", flags->conversion) && len < flags->right_aligned)
...
static char	*ft_create_prefix(char *prefix, char *s, t_flags *flags, size_t len)
{
	...
	else
	{
		...
		if (ft_strchr("di", flags->conversion) && flags->leading_zeros == 1
...
static char	*ft_apply_left_aligned(char *prefix, char *s, t_flags *flags)
{
	...
	if (flags->conversion == 'c' && len == 0)
		len = 1;
	len += (int) ft_strlen(prefix);
	if (ft_strchr("cspdiuxX", flags->conversion) && len <= flags->left_aligned)
	{
		tmp1 = ft_add_char(' ', flags->left_aligned - len);
		if (flags->conversion == 'c' && s[0] == '\0')
		{
			tmp2 = (char *) ft_calloc(1, (flags->left_aligned + 1));
			ft_memcpy((void *) &tmp2[1], (void *) tmp1, flags->left_aligned);
		}
		else
